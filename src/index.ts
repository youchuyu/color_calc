...
class ColorCalc {
  ...
  delta_Eab2000(labA: Lab, labB: Lab) {
    ...
    mean_Cab =
      (this.chroma(labA[1], labA[2]) + this.chroma(labB[1], labB[2])) / 2;
    ...
    let CC1, CC2; //两样本的彩度值
    CC1 = this.chroma(aa1, bb1);
    CC2 = this.chroma(aa2, bb2);
    let hh1, hh2; //两样本的色调角
    hh1 = this.hueAngle(aa1, bb1);
    hh2 = this.hueAngle(aa2, bb2);

    delta_LL = LL1 - LL2;
    delta_CC = CC1 - CC2;
    delta_hh = this.hueAngle(aa1, bb1) - this.hueAngle(aa2, bb2);
    delta_HH =
  ...
  harmony(labA: Lab, labB: Lab) {
    let deltaL = labA[0] - labB[0];

    let L_sum = labA[0] + labB[0];

    let deltaH =
      this.hueAngle(labA[1], labA[2]) - this.hueAngle(labB[2], labB[2]);
    let deltaC = this.chroma(labA[1], labA[2]) - this.chroma(labB[1], labB[2]);
    let h_C =
      0.04 +
      0.53 *
        tanh(
    ...
    let h_sy1 = this.h_sy(labA);
    let h_sy2 = this.h_sy(labB);
    let h_H = h_sy1 + h_sy2;
  ...
  fitness(labA: Lab, labB: Lab, a: number) {
    let delta = this.delta_Eab2000(labA, labB);
    let harmony = this.harmony(labA, labB);
    return delta + a * harmony;
  }

  fitnessAll(entity: Lab[], factor: number) {
    let lowestGrade = 1000;
    for (let i = 0; i < entity.length; i++) {
      for (let j = i + 1; j < entity.length; j++) {
        let grade = this.fitness(entity[i], entity[j], factor);
  ...
  getDeltaAndHarmony(entity: Lab[]) {
    console.log(entity);
    let delta = Infinity;
    let harmony = Infinity;
    for (let i = 0; i < entity.length; i++) {
      for (let j = i + 1; j < entity.length; j++) {
        delta = Math.min(delta, this.delta_Eab2000(entity[i], entity[j]));
        harmony = Math.min(harmony, this.harmony(entity[i], entity[j]));
        console.log("harmony", this.harmony(entity[i], entity[j]));
      }
    }
  ...
  getSeqDelta(palette: Lab[]) {
    ...
    for (let i = 1; i < palette.length; i++) {
      delta_arr.push(
        Math.round(this.delta_Eab2000(palette[i - 1], palette[i]))
      );
    }
    ...
    let SD = Math.round(
      Math.sqrt(
        delta_arr.reduce((p, c) => p + (c - avg) ** 2) / delta_arr.length
      )
    );
  ...
  h_sy(lab: Lab) {
    ...
    let h_ab = this.hueAngle(lab[1], lab[2]);
    let c_ab = this.chroma(lab[1], lab[2]);
  ...
  //彩度计算
  chroma(a: number, b: number) {
    return Math.pow(a * a + b * b, 0.5);
  }
  //色调角计算
  hueAngle(a: number, b: number) {
    let h = 0;
    let hab = 0;
